...
1. 제목이 **`NetWork-IP Flask 5000`** 인 검은 CMD 창이 떠 있어야 합니다. **이 창을 닫으면 서버가 꺼집니다.**
2. 브라우저에서 **`http://127.0.0.1:5000`**  
3. Wi‑Fi 전용 화면: **`http://127.0.0.1:5000/wifi`**

배치 파일 시작 시 콘솔에 **같은 Wi‑Fi용 `http://<IPv4>:5000`** 주소가 함께 출력됩니다. 휴대폰 등에서 쓰려면 Windows **방화벽에서 포트 5000 인바운드 허용**이 필요할 수 있습니다.
...
정상 기동 시 터미널에 Flask가 리스닝한다는 메시지가 나오고, 브라우저로 **`http://127.0.0.1:5000`** 을 엽니다.

## 4. 포트 바꾸기

기본 포트는 **5000** 입니다. 다른 포트를 쓰려면 환경 변수를 설정한 뒤 `app.py`를 실행합니다.
...
- 다른 프로그램이 5000을 쓰 중입니다. 위 **4. 포트 바꾸기**로 다른 포트를 지정하거나, 기존에 떠 있는 Flask/CMD 창을 종료합니다.
...
- PC 방화벽 **인바운드 규칙**에서 TCP **5000**(또는 바꾼 포트) 허용
- 주소는 **`http://127.0.0.1`** 이 아니라 PC의 **`http://192.168.x.x:5000`** (배치 파일 출력 또는 `ipconfig`로 확인)
