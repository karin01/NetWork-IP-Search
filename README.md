...
> **GitHub Pages** 루트는 **소개 웹 페이지**(`site-landing/`), 상세 **기술 문서**는 **`/manual/`** 경로(VitePress)입니다. 그래프·스캔 대시보드는 `python app.py`(또는 배치 파일) 실행 후 **`http://127.0.0.1:8500`** 에서 엽니다.
...
브라우저에서 `http://127.0.0.1:8500` 접속

### 접근 토큰·웹 설정·민감 파일 (선택)

- **`NETWORK_IP_SEARCH_TOKEN`** — 환경 변수로 설정하면 API·HTML 페이지 접근에 토큰이 필요합니다. 브라우저는 처음 `http://127.0.0.1:8500/?token=값` 으로 열거나, API 호출 시 헤더 `X-NetWork-IP-Token` 을 붙입니다. (`/api/health`, `/api/build-info`, 정적 `/static` 은 예외)
...
2. **`127.0.0.1` 또는 `localhost`로 다른 PC에서 접속한 경우**  
   `127.0.0.1`은 **그 PC 자기 자신**만 가리킵니다. 다른 컴퓨터에서는 **`https://karin01.github.io/NetWork-IP-Search/`** 처럼 **공개 주소**를 써야 하고, **대시보드(Flask)** 는 서버를 켠 PC의 **`http://그PC의내부IP:8500`** 으로 접속해야 합니다.
...
| **같은 집·사무실 Wi‑Fi에서 휴대폰으로 대시보드** | 대시보드를 켠 PC의 **사설 IP**로 접속: `http://192.168.x.x:8500` . Windows **방화벽에서 포트 8500 인바운드 허용**이 필요할 수 있습니다. 앱은 이미 `host=0.0.0.0` 으로 떠서 같은 망 접속이 가능합니다. |
| **인터넷 어디서나 브라우저로 대시보드 (임시)** | [ngrok](https://ngrok.com/) 등으로 `ngrok http 8500` → 발급된 **https URL**을 다른 PC에 입력. (보안·노출 주의, 테스트용에 적합) |
