...
- `site-landing/index.html` — Flask 실행 요약 카드, 스위치(Cisco·Juniper·HPE) 요약 + `manual/guide/*.html` 링크, 내비에 서버/스위치.
- `docs/guide/switch-ports.md` — IF-MIB·인벤토리·API·벤더별 절차·트러블슈팅.

## 대시보드를 “웹에서” 보는 해결책 (Pages로는 불가)

- 같은 Wi‑Fi: `http://PC사설IP:8500` + 방화벽 허용  
- 임시 외부: ngrok 등 터널  
...
## 2026-04-11 — Portfolio 수준 랜딩·다른 PC 테스트 안내

- `site-landing/index.html` · `styles.css`: 2열 히어로, 회전 링·글로우 오브, 상단 Home 밑줄 내비, 왼쪽 레일 SVG 아이콘.
- **공개 URL** 섹션 + **주소 복사** 버튼: [Portfolio](https://karin01.github.io/Portfolio/)처럼 인터넷만 있으면 어떤 PC/폰에서도 `github.io` 로 열 수 있음을 명시.
